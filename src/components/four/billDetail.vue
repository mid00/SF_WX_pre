            ...
            <div class="haveData" v-if="!close">
                ...
                <div class="item" v-for="(item,index) of fills" :key="index">               <!-- 1 层 item: {} -->
                    ...
                    <div class="content" v-for="(itemDetails,i) of item.items" :key="i">    <!-- 2 层 -->
                        ...
                        <span class="text">{{itemDetails.tag}}</span>           <!-- 名 -->
...
<!-- 复选 -->           <van-checkbox-group     v-if = "(itemDetails.type == '2')"    v-model = "itemDetails.value">
                            ...
                            <!-- <van-checkbox class="itemHeight"    @click="clickSet( [ index, i, `${a}`])" name="√"    v-show="!( (aItem == '')||(aItem == ';') )"   v-for="(aItem,a) of itemDetails.preselection_input" :key="a">{{aItem}}</van-checkbox> -->
...
<!-- 单选 -->           <van-radio-group        v-if = "(itemDetails.type == '3')"    v-model = "itemDetails.value">
                            ...
                            <!-- <van-radio class="itemHeight"       @click="clickSet( [ index, i, a])"   name="√"       v-show="!( (aItem == '')||(aItem == ';') )"   v-for="(aItem,a) of itemDetails.preselection_input" :key="a">{{aItem}}</van-radio> -->
...
export default {
    ...
    created(){
        ...
        this.fill_id = this.$route.params.arr; // 接收路由传参
        this.getDetails();      // 获取表单
    },
    // updated(){ this.clickSet();},
    methods:{
            ...
            getMsg( url , obj ).then( result=>{
                ...
                if( result.status == 200 ){
                    if( result.data.status == 200 ){
                        ...
                        if( result.data.data.length == 0 ){ this.close = true; } // 判断响应 ---> 数组

                        this.fills = result.data.data.pages     // 获取结果 ---> 数组
                        this.fill_name = result.data.data.template_name
                        console.log( "预填单数组")
                        console.log( this.fills)
                        console.log( this.fill_name)


                        for(var i=0; i<this.fills.length; i++){                 // 给每一个对象加属性 ---> value
                            for(var j=0; j<this.fills[i].items.length; j++){    // 循环对象里的数组 items

                                // console.log(typeof this.fills[i].items[j].preselection_input)

                                var item = this.fills[i].items[j].type;

// 复选
                                if( item == 2){

                                    this.$set( this.fills[i].items[j], "value", []);
                                    this.fills[i].items[j].preselection_input = [ this.fills[i].items[j].preselection_input ];

                                    // var arr2 = this.fills[i].items[j].preselection_input.split(";");
                                    // if( arr2[0].length == 0){ // 如果 没有内容

                                    //     this.$set( this.fills[i].items[j], "preselection_input", [""]);
                                    //     this.$set( this.fills[i].items[j], "value", ["√"]);

                                    // }else if( arr2[ arr2.length - 1].length == 0){

                                    //     arr2.pop();
                                    //     this.fills[i].items[j].preselection_input = arr2;
                                    // }else{
                                    //     this.fills[i].items[j].preselection_input = arr2;
                                    // }
                                }
// 单选
                                    ...
                                    // var arr3 = this.fills[i].items[j].preselection_input.split(";");
                                    // if( arr3[0].length == 0){

                                    //     // this.fills[i].items[j].type = 0;
                                    //     this.fills[i].items[j].preselection_input = [""];
                                    //     this.fills[i].items[j].value = "√";

                                    // }else if( arr3[ arr3.length - 1].length == 0){

                                    //     this.fills[i].items[j].preselection_input = this.fills[i].items[j].preselection_input.split(";");
                                    //     this.fills[i].items[j].preselection_input.pop();
                                    // }else{
                                    //     this.fills[i].items[j].preselection_input = arr3;
                                    // }
                                }
...
// 下拉列表
                                    ...
                                    this.$set( this.fills[i].items[j], "value", "");
                                    var arr4 = this.fills[i].items[j].preselection_input.split(";");
                                    ...
                                    }else if( arr4[ arr4.length - 1].length == 0){

                                        this.fills[i].items[j].preselection_input = this.fills[i].items[j].preselection_input.split(";");
                                        this.fills[i].items[j].preselection_input.pop();
                                    }else{
                                        this.fills[i].items[j].preselection_input = arr4;
                                    }
                                }
// 性别
                                else if( item == 19){
